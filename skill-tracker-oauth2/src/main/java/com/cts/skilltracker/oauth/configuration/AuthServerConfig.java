package com.cts.skilltracker.oauth.configuration;
...
public class AuthServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		clients.inMemory().withClient(clientId).secret(new BCryptPasswordEncoder(12).encode(clientSecret))
				.scopes(SCOPE_READ, SCOPE_WRITE, SCOPE_ADMIN)
				.authorizedGrantTypes(grantTypePassword, grantTypeRefreshToken)
				.accessTokenValiditySeconds(tokenValidity).refreshTokenValiditySeconds(refreshTokenValidity);
	}
}
